...
struct uinit {
  ...
  uinit() : type(init_type::NONE) { }
  uinit(R _const) : type(init_type::CONST), const_val(_const) { }
  uinit(random_state *rs, R _lo, R _hi) : type(init_type::RANDOM), rndst(rs), rnd_lo(_lo), rnd_hi(_hi) { }
  uinit(R _init, R _delta, R mod) : type(init_type::SEQ), seq_init(_init), seq_delta(_delta), seq_mod(mod) { }
...
template <typename R>
static uinit<R> init_none() {return uinit<R>();}
template <typename R>
static uinit<R> init_const(R _const) {return uinit<R>(_const);}
template <typename R>
static uinit<R> init_seq(R _init, R _delta = (R)1, R _mod = 0) {
  return uinit<R>(_init, _delta, _mod);
}
template <typename R>
static uinit<R> init_random(
  random_state &_rs,
  R _rnd_lo = std::numeric_limits<R>::min(),
  R _rnd_hi = std::numeric_limits<R>::max())
{
    return uinit<R>(&_rs, _rnd_lo,_rnd_hi);
}
...
public:
  ...
  template <typename R>
  umem(size_t _elems, const uinit<R> &i)
    : umem<E>(_elems)
  {
    apply<R>(i);
  }
